<template>
  <article class="group relative -mx-2 rounded-md px-2 py-1.5 transition-colors duration-150 hover:bg-[#f8f8fa]">
    <div class="flex gap-2.5">
      ...
      <div class="min-w-0 flex-1">
        ...
        <div
          ...
          class="mt-2 inline-flex min-w-[260px] items-center justify-between gap-3 rounded-[10px] border border-[#e5e7eb] bg-[#f8f8fa] px-3 py-2"
        >
          <div class="flex items-center gap-2.5">
            <span class="inline-flex h-7 w-7 items-center justify-center rounded-md bg-violet-100 text-violet-400">
            ...
            <div>
              <p class="max-w-[220px] truncate text-[13px] font-medium text-[#1d1c1d]">{{ message.attachment.name }}</p>
              <p class="text-[12px] text-[#8f95a3]">{{ message.attachment.size }}</p>
            </div>
          </div>
          <button type="button" class="inline-flex h-6 w-6 items-center justify-center rounded-md text-[#8f95a3] transition-colors hover:bg-white hover:text-[#616061]">
            <svg viewBox="0 0 20 20" class="h-4 w-4 fill-current">
    ...
    <div class="pointer-events-none absolute right-2 top-2 opacity-0 transition-opacity duration-150 group-hover:pointer-events-auto group-hover:opacity-100">
      <div class="inline-flex items-center gap-0.5 rounded-md border border-[#e5e7eb] bg-white p-1">
        <button type="button" class="inline-flex h-6 w-6 items-center justify-center rounded text-[#94a3b8] hover:bg-[#f8f8fa] hover:text-[#475569]" title="Add reaction">
          <svg viewBox="0 0 20 20" class="h-4 w-4 fill-current">
        ...
        <button
          type="button"
          class="inline-flex h-6 w-6 items-center justify-center rounded text-[#94a3b8] hover:bg-[#f8f8fa] hover:text-[#475569]"
          title="Reply in thread"
          @click="openThreadPanel"
        >
          <svg viewBox="0 0 20 20" class="h-4 w-4 fill-current">
        ...
        <button type="button" class="inline-flex h-6 w-6 items-center justify-center rounded text-[#94a3b8] hover:bg-[#f8f8fa] hover:text-[#475569]" title="Save">
          <svg viewBox="0 0 20 20" class="h-4 w-4 fill-current">
