<template>
  <header class="relative flex h-[72px] items-center justify-between border-b border-[#e5e7eb] bg-white px-4">
    <div>
      <div class="flex items-center gap-2">
        <h1 class="text-[18px] font-bold leading-6 text-[#1D1C1D]">#{{ channel.name }}</h1>
        <span class="text-[13px] font-medium text-[#8f95a3]">{{ channel.members }} members</span>
      </div>
      <p class="mt-1 text-[13px] font-normal leading-[18px] text-[#6B6F76]">{{ channel.description }}</p>
    </div>

    <div class="flex items-center gap-2 text-[#6b7280]">
      <button class="inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[#6b7280] transition hover:bg-[#f8f8fa] hover:text-[#374151]">
        <svg viewBox="0 0 20 20" class="h-[14px] w-[14px] fill-current">
      ...
      <button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded-md transition hover:bg-[#f8f8fa]" @click="openSearch">
      ...
      <button type="button" ref="notificationButtonRef" class="relative inline-flex h-7 w-7 items-center justify-center rounded-md transition hover:bg-[#f8f8fa]" @click="toggleNotifications">
        <svg viewBox="0 0 20 20" class="h-4 w-4 fill-current">
        ...
        <span class="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full bg-indigo-500" />
      </button>
    ...
    <div v-if="isNotificationOpen" ref="notificationRef" class="absolute right-4 top-[64px] z-30">
